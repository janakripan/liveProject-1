...
import DescriptionRenderer from "../AdminDashboard/Shared/DescriptonRenderer";
    ...
    >
      <div className="w-full h-full p-4">
        <div className="w-full h-fit flex flex-row justify-between items-center  ">
          ...
          <button className=" bg-[#30D158] cursor-pointer font-satoshi font-bold text-base text-white flex flex-row items-center justify-center gap-x-2.5 px-4 py-3 rounded-lg hover:scale-105 transition-transform duration-300 ">
            <FaEdit />
        ...
        <div className="w-full h-[85vh]  mt-8 flex flex-row gap-y-5">
          ...
          <div className="w-1/5 h-full min-h-full overflow-y-auto">
            <PreviewSidebar data={project.modules} scrollContainerRef={scrollContainerRef} />
          </div>
          ...
          ref={scrollContainerRef}
           className="w-4/5  h-full min-h-full px-4 overflow-hidden overflow-y-scroll ">
            {project.modules?.map((module) => (
              <section className="w-full h-fit pb-5 " key={module.module_id}>
                {module.sub_modules.map((sub) => (
                  <section
                    key={sub.sub_module_id}
                    className="w-full h-full max-h-[80vh] min-h-[60vh] bg-yellow-500 flex flex-row pb-5 my-5 border-b overflow-hidden border-[#E6E1E280] gap-5"
                  ...
                  >
                    ...
                    <div className="w-full lg:w-1/2 h- flex flex-col gap-5 overflow-y-scroll no-scrollbar p-4 ">
